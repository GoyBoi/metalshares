...
const Footer = ({ id = 'footer' }: FooterProps) => {
  return (
    <footer id={id} className="bg-everglade text-spring-wood pt-16 pb-8">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          <div>
            <h3 className="text-xl font-bold font-serif mb-4">MetalShares</h3>
            <p className="text-surf-crest mb-4">
            ...
            <div className="flex space-x-4">
              {[1, 2, 3, 4].map((item) => (
                <a 
                  key={item} 
                  href="#" 
                  className="text-surf-crest hover:text-de-york transition-colors duration-300"
                  aria-label={`Social media link ${item}`}
                >
                  <div className="bg-surf-crest/20 w-10 h-10 rounded-full flex items-center justify-center">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {['Gold Bars', 'Silver Coins', 'Platinum', 'Palladium', 'IRA Options'].map((item) => (
                <li key={item}>
                  <a href="#" className="text-surf-crest hover:text-de-york transition-colors duration-300">
                    {item}
                  </a>
                </li>
              ))}
        ...
        <div className="pt-8 mt-8 border-t border-como/30 text-center text-surf-crest text-sm">
